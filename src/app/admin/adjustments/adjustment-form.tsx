...
export default function AdjustmentForm({ recentAdjustments }: { recentAdjustments: AdjTx[] }) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
          ...
          {selectedItem && (
            <>
              ...
              {delta !== null && physicalStock !== '' && (
                <div className={`rounded-md p-3 text-sm ${delta === 0 ? 'bg-gray-50 text-gray-600' : delta > 0 ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
      ...
      {/* Recent adjustments */}
      <div>
        <h2 className="mb-3 text-base font-semibold text-gray-900">Penyesuaian Terbaru</h2>
        {recentAdjustments.length === 0 ? (
          <p className="text-sm text-gray-500">Belum ada penyesuaian.</p>
        ) : (
              ...
              return (
                <div key={adj.id} className="card text-sm">
                  <div className="flex items-start justify-between">
                    <div>
                      <code className="text-xs">{adj.transaction_number}</code>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-400">{dtf.format(new Date(adj.transaction_at))}</p>
                    </div>
                  </div>
                </div>
              )
      ...
      {showConfirm && selectedItem && delta !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" role="dialog" aria-modal="true">
          <div className="mx-4 w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-gray-900">Konfirmasi Penyesuaian</h2>
            <div className="mt-3 space-y-2 text-sm text-gray-700">
              ...
              <p><span className="font-medium">Selisih:</span> <span className={delta >= 0 ? 'text-green-700' : 'text-red-700'}>{delta >= 0 ? '+' : ''}{delta.toLocaleString('id-ID')}</span></p>
