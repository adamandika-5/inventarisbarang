...
import { createItem, generateAutoBarcode } from '../actions'
...
export default function NewItemForm({ categories, units }: NewItemFormProps) {
  ...
  const handleGenerateBarcode = async () => {
    ...
    if (barcodeValue.trim()) {
      const confirmed = window.confirm('Barang ini sudah memiliki nilai barcode. Yakin ingin mengganti dengan barcode baru?')
      if (!confirmed) return
    }
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                {barcodeFormat === 'CODE128' && (
                  ...
                  >
                    {isGeneratingBarcode ? 'Membuat…' : barcodeValue.trim() ? 'Buat Ulang' : 'Buat Otomatis'}
                  </button>
                )}
              </div>
              {genError && (
